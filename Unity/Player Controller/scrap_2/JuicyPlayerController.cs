...
namespace ONYX {
    public class JuicyPlayerController : MonoBehaviour {
        ...
        [SerializeField] private float mouse_sensitivity = 25f;
        ...
        [Header ("Walk")]
        [SerializeField] private float walk_speed = 7.5f;

        [Header ("Run")]
        [SerializeField] private float run_speed = 15f;
        ...
        [Header ("Slope Check")]
        [SerializeField] private float slope_check_distance = 2f;

        [Header ("Slide")]
        [SerializeField] private float ground_drag = 2f;
        [SerializeField] private float slide_speed = 0.5f;
        [SerializeField] private float slide_check_distance = 2f;

        [Header ("Debug (read-only)")]
        [ReadOnly] [SerializeField] private PlayerState state;
        [ReadOnly] [SerializeField] private PlayerState last_state;
        [ReadOnly] [SerializeField] private Vector3 velocity;
        [ReadOnly] [SerializeField] private Vector3 slide_velocity;
        ...
        private void Update(){
            ...
            CalculateSlideVelocity(state == PlayerState.Crouching, state != last_state && state == PlayerState.Crouching);

            controller.Move((velocity + slide_velocity) * Time.deltaTime);
        ...
        private Vector3 AlignMoveWithGround(Vector3 _move){
            if (Physics.Raycast(transform.position, Vector3.down, out RaycastHit hit, slope_check_distance, ground_mask)){
                Vector3 ground_normal = hit.normal;

                Vector3 perpendicular = Vector3.Cross(_move, ground_normal);

                _move = Vector3.Cross(ground_normal, perpendicular).normalized * _move.magnitude;
            }
        ...
        private void CalculateSlideVelocity(bool _crouching, bool _just_crouched){
            if(_just_crouched){
                slide_velocity = controller.velocity;
            }

            if (_crouching){
                if (Physics.Raycast(transform.position, Vector3.down, out RaycastHit hit, slide_check_distance, ground_mask)){
                    Vector3 groundNormal = hit.normal;

                    float groundAngle = Vector3.Angle(Vector3.up, groundNormal);

                    Vector3 slideDirection = Vector3.Cross(Vector3.Cross(groundNormal, Vector3.down), groundNormal).normalized;

                    slide_velocity += slideDirection * Mathf.Sin(groundAngle * Mathf.Deg2Rad) * slide_speed;
                }
            }

            slide_velocity *= 1 - ground_drag * Time.deltaTime;
        }
